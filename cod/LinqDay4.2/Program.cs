    ...
    class Program
    {
        static void Main()
        {
            List<Client> clients = new List<Client>
            {
                ...
                new Client {Id = 6, Name = "Dimon"},
                new Client {Id = 7, Name = "Vova"}
            };
            ...
            {
                ClientName = c.Name,
                Orders = grp.Select(x => x.Product).ToList()
            });

            System.Console.WriteLine("Group Join, Все клиенты (даже те у которых нету заказов)");

            foreach (var entry in left)
            {
                System.Console.Write($"{entry.ClientName}: ");
                if (!entry.Orders.Any())
                {
                ...
                {
                    System.Console.WriteLine(string.Join(", ", entry.Orders));
                }
            }
        }
    }
